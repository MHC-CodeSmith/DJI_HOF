...
def main():
    # Fix path resolution
    # Current: src/visualization/flight_map.py
    # Root:    ../../
    base_dir = Path(__file__).resolve().parent.parent.parent
    
    # Check both potential locations for the CSV (legacy or new structure)
    # 1. Project Root / extracted_metadata (Legacy)
    csv_path = base_dir / "extracted_metadata" / "all_metadata_consolidated.csv"
    
    # 2. If not found, check data/processed/extracted_metadata (New)
    if not csv_path.exists():
        csv_path = base_dir / "data" / "processed" / "extracted_metadata" / "all_metadata_consolidated.csv"

    if not csv_path.exists():
        # Fallback to just data/processed if consolidation happened there
        csv_path = base_dir / "data" / "processed" / "all_metadata_consolidated.csv"

    if not csv_path.exists():
        raise FileNotFoundError(f"Consolidated CSV not found at {csv_path}")
