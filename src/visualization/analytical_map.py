#!/usr/bin/env python3
"""
Generates an interactive analytical map (Vegetation Health).
Based on the 'Berkay' version which uses Leaflet Heatmaps and Per-Track layers.
"""
...
def calculate_bounds(points):
    ...
    if not lats:
        return {"center_lat": 0, "center_lon": 0, "min_lat": 0, "max_lat": 0, "min_lon": 0, "max_lon": 0}
    ...
    for track_id, pts in track_groups.items():
        ...
        var heat_{safe_id} = L.heatLayer(
          ...
          {{
            ...
            maxZoom: 19,
            minOpacity: 0.35,
            gradient: {{0.2: 'red', 0.5: 'yellow', 1.0: '#00ff00'}}
          }}
        );
        ...
        # overlays_entries.append(f"'Points {track_id}': pontos_{safe_id}") # Optional: comment out points to avoid clutter
...
def main():
    # Fix path resolution (Robust logic)
    base_dir = Path(__file__).resolve().parent.parent.parent
    
    # Check legacy and new paths
    # 1. Project Root / extracted_metadata
    csv_path = base_dir / "extracted_metadata" / "all_metadata_with_health.csv"
    
    # 2. If not found, check data/processed/extracted_metadata
    if not csv_path.exists():
        csv_path = base_dir / "data" / "processed" / "extracted_metadata" / "all_metadata_with_health.csv"

    if not csv_path.exists():
        # Fallback
        csv_path = base_dir / "data" / "processed" / "all_metadata_with_health.csv"

    if not csv_path.exists():
        print(f"❌ File not found: {csv_path}")
        print("   Run src/processing/sync_video_csv.py first")
        return
    
    print("🚀 Generating analytical map (Leaflet Heatmap + Tracks)\n")
    print("📖 Loading data...")
    points = load_metadata_with_health(csv_path)
    
    if not points:
        print("❌ No points loaded/found!")
        return
